...
## Professor attention data (DOI based)
...
dbWriteTable(con, "almetric_prof_attention", prof_attention_list, row.names=FALSE, append=TRUE)
```
...
for (i in 1:length(batches)){
  ...
  if (!all(is.na(tweet_info_full))){
    ...
    if (! dbExistsTable(con, "twitter_mentions")){
      # write to the database
      dbWriteTable(con, "twitter_mentions", tweet_info_full, row.names=FALSE, append=TRUE) 
      # otherwise, append
    }else{
      # check fields in the existing table
      fields <- dbListFields(con, "twitter_mentions")
      ...
      dbAppendTable(con, "twitter_mentions", tweet_info_full, row.names=NULL, append=TRUE) 
    }
  }
...
Overwrite the data:
```{r}
saveRDS(nwo_grants_narcis, "processed_data/nwo_grants.RDS")
```
