...
# Average numbers of mentions and comparison per gender (Figure 1A)
...
significance_testing$x <- c(1,2,3,4)
significance_testing$xmin <- c(0.75,1.75,2.75,3.75)
significance_testing$xmax <- c(1.25,2.25, 3.25,4.25)
...
average_twitter <- field_comparisons_plot %>%
   ...
   stat_pvalue_manual(
    ...
    scale_y_continuous(limits = c(0, 1100), breaks = seq(0, 1100, by = 275))+
   ylab("Average mentions over lifetime")+
   labs(color = "Inferred gender")+
   theme_minimal_vgrid()+
  theme(plot.title = element_text(size = 10),
...
# Women's representation (Figure 1B)
...
Get shares of women among top n% scientists per attention type:
```{r}
...
share_women_field_2023 <- women_field_2023 %>%
  ungroup()%>%
  select(profile_id, year, inferred_gender, general_field, `News`:`Twitter`)%>%
...
repr_attn_10 <- share_women_field_2023 %>%
  ...
  ggtitle("Top 10% by received attention")+
  theme_minimal_vgrid()+
  theme(plot.title = element_text(size = 10),
        axis.text.y = element_text(size = 10),
        axis.title.y = element_text(size = 11),
...
repr_attn_20 <- share_women_field_2023 %>%
  ...
  labs(color = "Attention type",
       shape = "Attention type")+
  ggtitle("Top 20% by received attention")+
  theme_minimal_vgrid()+
  theme(plot.title = element_text(size = 10),
        axis.text.y = element_text(size = 10),
        axis.title.y = element_text(size = 11),
...
ggsave2(
  filename = "results/main/Figure_1_palette.png",
  plot = combi_plot_2,
  width = 11,
  height = 7,
  units = c("in"),
...
ggsave2(
  filename = "results/main/Figure_2_palette.png",
