...
for (i in 1:length(narcis_ids)){
  ...
  oa_ids <- dbGetQuery(conn = con, 
                          statement = paste0("select * from oa_identifier_table where \"profile_id\"='", id, "'"))
  
  oa_id_list <- oa_ids$oa_id
  
  # load the relevant data from the publications table in our database
  prof_pubs <- dbGetQuery(conn = con, 
                          statement = paste0("select * from oa_pubs where \"profile_id\"='", id, "'"))
  ...
  prof_year_citations <- prof_pubs %>% 
    filter(!is.na(publication_year) & !is.na(counts_by_year_year_year) & !is.na(language))%>%
    group_by(author_position, counts_by_year_year_year, language)%>%
    summarise(cited_by = sum(counts_by_year_year_cited_by_count))%>%
    arrange(counts_by_year_year_year) %>%
    group_by(author_position, language) %>%
  ...
  latest_citation <- prof_year_citations %>%
    group_by(author_position, language)%>%
    slice(which.max(counts_by_year_year_year))
  ...
  prof_coauthors <- dbGetQuery(conn = con, 
                          statement = paste0("select * from coauthor_info where \"profile_id\"='", id, "'"))
  ...
  # do the same for affiliations, pull them first
  prof_affiliations <- dbGetQuery(conn = con, 
                          statement = paste0("select * from affiliation_info where \"profile_id\" in (", id, ")"))
  
  prof_affiliations_per_year <- prof_affiliations %>%
    group_by(institution_country_code, year)%>%
    summarise(institutions = n())%>%
    mutate(institutions_total = cumsum(institutions))
