...
average_online <- field_comparisons_plot %>%
  ...
  scale_y_continuous(limits = c(0, 212), breaks = seq(0, 212, by = 53))+
   stat_pvalue_manual(
     significance_testing,
     y.position = c(11, 43, 209, 140, 112),
     label.size = 3,
     coord.flip = TRUE,
     tip.length = 0.005,
     label = "{online_total}",
...
average_twitter <- field_comparisons_plot %>%
   ...
   stat_pvalue_manual(
     significance_testing,
     y.position = c(50, 200, 1120, 850, 440),
     label.size = 3,
     coord.flip = TRUE,
     tip.length = 0.005,
     label = "{twitter_total}",
...
repr_field_2023 <- prof_panel_filter %>%
  ...
  filter(!is.na(scopus_field_overall))%>%
  group_by(scopus_field_overall, inferred_gender)%>%
  summarise(n = n())%>%
...
women_field_2023 <- women_field_2023 %>%
  filter(!is.na(scopus_field_overall))%>%
  select(profile_id, year, inferred_gender, scopus_field_overall, count_pubs_total,
...
share_women_field_2023 <- women_field_2023 %>%
  ungroup()%>%
  select(profile_id, year, inferred_gender, scopus_field_overall, `News`:`Twitter`)%>%
  pivot_longer(`News`:`Twitter`)%>%
  group_by(scopus_field_overall, inferred_gender, name, value)%>%
  summarise(n = n())%>%
  pivot_wider(values_from = n, names_from = inferred_gender)%>%
  replace(is.na(.), 0)%>%
...
share_women_field_2023 <- merge(share_women_field_2023,
                                repr_field_2023[c("scopus_field_overall", "share_women_field")],
                                by = "scopus_field_overall")
...
women_field_2023_attn_share <- women_field_2023 %>%
  ...
  select(scopus_field_overall, `Publications`:`Twitter`)%>%
  pivot_longer(!scopus_field_overall, names_to = "name", values_to = "share_women")
```
Combine the field representation with the attention representation:
```{r}
