...
```{r}
prof_panel_filter$general_field <- factor(prof_panel_filter$general_field,
                                               levels = c("STEM",
                                                          "Medicine",
                                                          "Social sciences",
                                                          "Arts & Humanities"))
...
legend_total_attention <- get_legend(
  # create some space to the left of the legend
  totals_news
)

total_attention <- plot_grid(
  totals_news + theme(legend.position="none"), 
  totals_online_news+ theme(legend.position="none"), 
  totals_twitter+ theme(legend.position="none"),
  legend_total_attention,
...
# #save the plot
# ggsave2(
#   filename = "results/supplement_figures/Figure_S1.png",
...
```{r}
final_numbers <- prof_panel_filter %>%
  group_by(profile_id)%>%
  filter(year == max(year) & !is.na(overall_field))
```


Plot Lorenz curves and calculate Gini coefficients within each field.
```{r fig.width=8, fig.height=10}
# prepare the attention distributions
lorenz_field <- final_numbers %>%
...
gini_field$labels <- rep(c("Printed news", "Online news", "Twitter/X"), 4)
...
#     unit = "in",
#     res = 600,
#     file="results/supplement_figures/lorenz_curves.png")
# lorenz_field_plot
# dev.off()
...
prof_panel_filter$any_grant <- as.factor(ifelse(prof_panel_filter$nwo_grants > 0|prof_panel_filter$erc_grants, 1, 0))
prof_panel_filter$any_grant_l <- as.factor(ifelse(prof_panel_filter$nwo_grants_l > 0|prof_panel_filter$erc_grants_l, 1, 0))
```
...
```{r overall_women, fig.width=12, fig.height = 4}
...
capture_gender_only <- capture_gender %>% 
  ...
  ggtitle("A    Share of total attention captured by women")+
  scale_color_manual(labels = c("Printed news", "Online news", "Twitter/X"), values = c("#F8766D", "#00BA38", "#619CFF"))+
...
```{r overall_capture, fig.width=12, fig.height = 4}
news_capture <- prof_panel_filter %>%
  filter(year >= 2012)%>%
...
top_10_capture <- top_10_capture_overall %>% 
  ...
  xlab("Year")+
  ylab("Share of attention")+
  ggtitle("B    Share of total attention captured by professors in top 10%")+
  scale_color_manual(labels = c("Printed news", "Online news", "Twitter/X"), values = c("#F8766D", "#00BA38", "#619CFF"))+
  facet_grid(.~general_field)+
  labs(color = "Attention type")+
  theme_minimal_vgrid()+
...
Gender and field:
```{r by_field_gender, fig.width=12, fig.height = 4}
news_capture <- prof_panel_filter %>%
  filter(year >= 2012)%>%
...
top_10_man_capture <- top_10_capture_field_gender %>% 
  ...
  xlab("Year")+
  ylab("Share of attention")+
  ggtitle("C    Share of total attention captured by the men in top 10%")+
  scale_color_manual(labels = c("Printed news", "Online news", "Twitter/X"), values = c("#F8766D", "#00BA38", "#619CFF"))+
  facet_grid(.~general_field)+
  labs(color = "Attention type")+
  theme_minimal_vgrid()+
...
top_10_woman_capture <- top_10_capture_field_gender %>% 
  ...
  xlab("Year")+
  ylab("Share of attention")+
  ggtitle("D    Share of total attention captured by the women in top 10%")+
  ...
  theme(plot.title = element_text(size = 12),
        ...
        legend.text=element_text(size=10))
```
Compile these into a large figure:
```{r fig.width=11, fig.height=11}
legend_capture <- get_legend(
  capture_gender_only + theme(legend.position = "bottom")
)


(capture_trends_all <- plot_grid(
  capture_gender_only + theme(legend.position="none"),
  top_10_capture + theme(legend.position="none"),
  top_10_man_capture + theme(legend.position="none"),
  top_10_woman_capture + theme(legend.position="none"),
  legend_capture,
  ncol = 1,
  nrow = 5,
...
# 
# ggsave2(
#   filename = "results/supplement_figures/inequality_change.png",
#   plot = capture_trends_all,
#   width = 11,
#   height = 11,
#   units = c("in"),
#   dpi = 600,
...
overall_comparisons[i, 2] <- round(t.test(count_pubs_total ~ inferred_gender, data=sources_check, paired=FALSE)$p.value, 5)
overall_comparisons[i, 3] <- round(t.test(cited_by_total_all ~ inferred_gender, data=sources_check, paired=FALSE)$p.value, 5)
overall_comparisons[i, 4] <- round(t.test(news_all_total ~ inferred_gender, data=sources_check, paired=FALSE)$p.value, 5)
overall_comparisons[i, 5] <- round(t.test(alt_online_all_total ~ inferred_gender, data=sources_check, paired=FALSE)$p.value, 5)
overall_comparisons[i, 6] <- round(t.test(alt_twitter_total ~ inferred_gender, data=sources_check, paired=FALSE)$p.value, 5)
  ...
  for (j in 1:length(fields)){
    ...
    if (length(unique(data$inferred_gender)) != 2){
      ...
      if (length(which(colSums(data[c("alt_online_all_total", "news_all_total", "alt_twitter_total", "cited_by_total_all", "count_pubs_total")]) == 0)) > 0){
        ...
        year_groups_field_comparisons[row_index+j, 2] <- field
        year_groups_field_comparisons[row_index+j, 3] <- nrow(data)
        year_groups_field_comparisons[row_index+j, 4] <- round(t.test(count_pubs_total ~ inferred_gender, data=data, paired=FALSE)$p.value, 5)
        year_groups_field_comparisons[row_index+j, 5] <- round(t.test(cited_by_total_all ~ inferred_gender, data=data, paired=FALSE)$p.value, 5)
        year_groups_field_comparisons[row_index+j, 6] <- round(t.test(news_all_total ~ inferred_gender, data=data, paired=FALSE)$p.value, 5)
        year_groups_field_comparisons[row_index+j, 7] <- round(t.test(alt_online_all_total ~ inferred_gender, data=data, paired=FALSE)$p.value, 5)
        year_groups_field_comparisons[row_index+j, 8] <- round(t.test(alt_twitter_total ~ inferred_gender, data=data, paired=FALSE)$p.value, 5)
      }
    }
  }
...
overall_comparisons_out <- overall_comparisons %>%
  ...
  mutate_at(vars(`online_total_ttest`, `total_ttest`, `twitter_total_ttest`), .funs = list(
    ~case_when(
      . <= 0.001 ~ '***',
      . <= 0.01 ~ '**',
      . <= 0.05 ~ '*',
...
year_groups_field_comparisons_out <- year_groups_field_comparisons_out %>%
  select(field, year_group, total_m:twitter_total_ttest)%>%
...
t_test_breakdown_news <- sources_check %>%
  select(inferred_gender, general_field, news_all_total, news_intl_total,
         news_national_total,news_regional_total, news_other_total)%>%
...
source_breakdown_news$news_type <- ordered(source_breakdown_news$news_type,
                                     levels = c("total",
                                                "intl",    
                                                "national",
                                                "regional",
...
source_breakdown <- sources_check %>%
  ...
  mutate(across(14:22, \(x) round(x, 2)))
...
source_breakdown_online_news <- source_breakdown_online_news %>%
   mutate(news_type = recode(news_type, 
                             ...
                             `local` = "General interest - local news",
...
covariate_list_news <- c(
  "news_all ~ news_all_l",
  "news_offline_all ~ news_offline_all_l",
...
covariate_names <- c(
  'news_all' = "All\n news",
  'news_offline_all' = "Offline\n news",
  'news_national'="National\n news (NL)",
  'news_regional' = "Regional\n news (NL)",
...
(news_gender_plot <- predictions_plot %>%
  ...
  labs(color = "Inferred gender")+
    ...
    facet_grid(.~as.factor(covariate), 
               scales="free", 
...
yaxis$y.position <-  ifelse(yaxis$covariate == "alt_online_all", yaxis$y.position + 1, yaxis$y.position + 0.8)
...
news_model_2019 <- lm_fitter_cl_robust(panel_dataset = prof_panel_filter,
                                       lm_formula = news_formula_main_model,
                                       year_cutoff_upper = 2019,
                                       year_cutoff_lower = 2012)
```
...
online_news_model_2019 <- lm_fitter_cl_robust(panel_dataset = prof_panel_filter,
                                              lm_formula = online_news_formula_main_model,
                                              year_cutoff_upper = 2019,
                                              year_cutoff_lower = 2012)
```
...
twitter_model_2019 <- lm_fitter_cl_robust(panel_dataset = prof_panel_filter,
                                              lm_formula = twitter_formula_main_model,
                                              year_cutoff_upper = 2019,
                                              year_cutoff_lower = 2012)
```
...
## Main models - log-transformed variables until 2020
...
gender_printed_overview$overall <- gender_printed_overview$neg + gender_printed_overview$pos
