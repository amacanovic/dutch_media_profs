...
And then make a list of unique names that appear at least 3 times in our dataset 
(which covers 84.6% of all people, and 22.7 k names)
```{r}
coauathor_name_counts <- coauthor_names %>%
  group_by(first_lowercase)%>%
  summarise(n = n())%>%
  filter(n > 3)

unique_names <- distinct(coauathor_name_counts, first_lowercase)
```
...
saveRDS(coauthor_name_gender  , "processed_data/coauthor_gender_genderize_all.RDS")
```

# Journalists from Mentions data
Get the journalist names where applicable:
```{r}
mentions_full_text <- dbReadTable(con, "mentions_full_text") 

# remove NAs
mentions_full_text_authors <- filter(mentions_full_text,
                                      !is.na(web_author))

# check for newspaper names
counts_of_authors <- mentions_full_text_authors %>%
  group_by(web_author)%>%
  summarise(n = n())
```
