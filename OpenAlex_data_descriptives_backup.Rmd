    ...
    if (length(prof)>1){
      ...
      if (! all(is.na(oa_pubs))){
        ...
        for (k in 1:nrow(oa_pubs)){
          ...
          if (all(c("id", "display_name", "author", "author_position", "so", "so_id", "doi", "language", "grants", "cited_by_count", "publication_year",
    "cited_by_api_url", "type", "profile_id", "concepts") %in%  colnames(pub))){
          ...
          if (!all(is.na(citations_year))){
            ...
            for (col in colnames(pub_mentions)){
              ...
              if (!all(is.na(col_data)) & "posted_on" %in% colnames(col_data)){
